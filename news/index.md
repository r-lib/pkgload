...
- [`load_all()`](https://pkgload.r-lib.org/reference/load_all.md) now
  supports explicit `debug` option to control using debug compiler flags
  ([\#224](https://github.com/r-lib/pkgload/issues/224),
...
- The `reset` argument of
  [`load_all()`](https://pkgload.r-lib.org/reference/load_all.md) is no
  longer supported because preserving the namespace requires unlocking
  its environment, which is no longer possible in recent versions of R.
  It should no longer be necessary as the performance issues caused by
  resetting the namespace were resolved a while ago.

- New experimental feature for generating a `compile_commands.json` file
  after each
  [`load_all()`](https://pkgload.r-lib.org/reference/load_all.md). This
  file is used by LSP servers such as clangd to provide intellisense
  features in your native files. To enable it, add this directive to
...
- [`load_all()`](https://pkgload.r-lib.org/reference/load_all.md) now
  includes a link to the exact location when loading failed
...
- Fixed a bug in
  [`shim_help()`](https://pkgload.r-lib.org/reference/help.md) where a
  complex `package =` argument evaluating to `NULL` would cause an error
  ([\#266](https://github.com/r-lib/pkgload/issues/266)).
...
- [`dev_topic_index()`](https://pkgload.r-lib.org/reference/dev_help.md)
  is now exported
...
- [`load_all()`](https://pkgload.r-lib.org/reference/load_all.md) no
  longer standardises version number in namespace metadata
  ([\#231](https://github.com/r-lib/pkgload/issues/231)).

- New
  [`pkg_version_raw()`](https://pkgload.r-lib.org/reference/packages.md)
...
- [`dev_topic_find()`](https://pkgload.r-lib.org/reference/dev_help.md)
  is now exported
  ([\#215](https://github.com/r-lib/pkgload/issues/215)).

- [`dev_help()`](https://pkgload.r-lib.org/reference/dev_help.md) will
  remind you to run
  [`pkgload::load_all()`](https://pkgload.r-lib.org/reference/load_all.md)
...
- [`dev_help()`](https://pkgload.r-lib.org/reference/dev_help.md) now
  works with an RStudio daily to deliver a rendered development
...
- [`load_all()`](https://pkgload.r-lib.org/reference/load_all.md) gains
  an `attach` argument set to `TRUE` by default
...
- [`unregister()`](https://pkgload.r-lib.org/reference/unload.md) is now
  exported. This is a gentler version of
  [`unload()`](https://pkgload.r-lib.org/reference/unload.md) which
...
- [`load_all()`](https://pkgload.r-lib.org/reference/load_all.md) now
  imports its dependencies lazily to avoid parallel installation issues
  ([\#89](https://github.com/r-lib/pkgload/issues/89)).
...
- New [`is_loading()`](https://pkgload.r-lib.org/reference/load_all.md)
  predicate to detect whether
  [`load_all()`](https://pkgload.r-lib.org/reference/load_all.md) is
  currently running
...
- [`load_all()`](https://pkgload.r-lib.org/reference/load_all.md) now
  restores S3 methods registered by third party packages
  ([\#163](https://github.com/r-lib/pkgload/issues/163)).

- [`load_dll()`](https://pkgload.r-lib.org/reference/load_dll.md) will
  now preserve the DLL name when loading instead of always using the
  package name. This allows packages to include DLL’s with different
...
- [`unload()`](https://pkgload.r-lib.org/reference/unload.md) no longer
  unregisters methods for generics of the package being unloaded. This
  way dangling references to generics defined in the stale namespace
  still work as expected (r-lib/vctrs#1341).

- [`load_all()`](https://pkgload.r-lib.org/reference/load_all.md) will
  now work for packages that have testthat tests but do not have
  testthat installed
...
- [`unload()`](https://pkgload.r-lib.org/reference/unload.md) no longer
  warns when it can’t unload a namespace.
...
- [`load_all()`](https://pkgload.r-lib.org/reference/load_all.md) now
  preserves existing namespaces in working order. In particular, it
...
- [`load_all()`](https://pkgload.r-lib.org/reference/load_all.md) no
  ...
  - This also makes
    [`load_all()`](https://pkgload.r-lib.org/reference/load_all.md) more
    predictable after a method has been removed from the package. It is
    now actually removed from the generic table. It would previously
    linger until R was restarted.

- If [`load_all()`](https://pkgload.r-lib.org/reference/load_all.md)
...
- [`dev_example()`](https://pkgload.r-lib.org/reference/dev_example.md)
...
- [`load_all()`](https://pkgload.r-lib.org/reference/load_all.md) now
  issues a warning if exported objects conflict with objects defined in
  the global environment
  ([\#112](https://github.com/r-lib/pkgload/issues/112))

- [`run_example()`](https://pkgload.r-lib.org/reference/dev_example.md)
...
- [`shim_question()`](https://pkgload.r-lib.org/reference/help.md) now
  works for topics from the R base package that are passed with the
...
- [`load_all()`](https://pkgload.r-lib.org/reference/load_all.md) now
  allows using explicitly qualified, exported names in test helpers
  ([@klmr](https://github.com/klmr),
  [\#95](https://github.com/r-lib/pkgload/issues/95)).

- [`load_all()`](https://pkgload.r-lib.org/reference/load_all.md) gains
  a `compile` argument which controls more finely whether to compile the
  code or not. The `recompile` argument is now deprecated and will be
  removed in a future version of pkgload.

## pkgload 1.0.1

CRAN release: 2018-10-11

- [`unload()`](https://pkgload.r-lib.org/reference/unload.md) now only
  removes S4 classes which were generated in the package being unloaded
  ([\#75](https://github.com/r-lib/pkgload/issues/75))

- [`help()`](https://pkgload.r-lib.org/reference/help.md) will no longer
  error when trying to load package level help
...
- [`load_dll()`](https://pkgload.r-lib.org/reference/load_dll.md) now
  fixed in R-devel ([\#77](https://github.com/r-lib/pkgload/issues/77)).
...
- [`load_all()`](https://pkgload.r-lib.org/reference/load_all.md) now
  updates imports of dependent packages when a package is reloaded
  ([\#59](https://github.com/r-lib/pkgload/issues/59)).

- [`load_all()`](https://pkgload.r-lib.org/reference/load_all.md) now
  assigns `DESCRIPTION/Depends` to `.Depends` object of package
  environment. ([@yiufung](https://github.com/yiufung) pkgload#61)

- [`load_all()`](https://pkgload.r-lib.org/reference/load_all.md) now
  attaches `testthat` if the `attach_testthat` option is `TRUE`. This
  allows [`load_all()`](https://pkgload.r-lib.org/reference/load_all.md)
  to more closely mimic the testing environment.
  ([\#56](https://github.com/r-lib/pkgload/issues/56))

- [`check_dep_version()`](https://pkgload.r-lib.org/reference/check_dep_version.md)
  and
  [`check_suggested()`](https://pkgload.r-lib.org/reference/check_suggested.md)
  are now exported.
...
- Package imports are now exported when using
  [`load_all()`](https://pkgload.r-lib.org/reference/load_all.md). This
  behavior can be disabled by using `load_all(export_imports = FALSE)`.

- The `as.package()` and `is.package()` functions have been removed.

- [`load_code()`](https://pkgload.r-lib.org/reference/load_code.md),
  [`load_data()`](https://pkgload.r-lib.org/reference/load_data.md),
  [`load_dll()`](https://pkgload.r-lib.org/reference/load_dll.md),
  [`load_all()`](https://pkgload.r-lib.org/reference/load_all.md),
  [`parse_ns_file()`](https://pkgload.r-lib.org/reference/parse_ns_file.md)
  all now take an explicit path rather than a path or a `package`
  object.

- [`imports_env()`](https://pkgload.r-lib.org/reference/imports_env.md),
  [`ns_env()`](https://pkgload.r-lib.org/reference/ns_env.md),
  [`pkg_env()`](https://pkgload.r-lib.org/reference/pkg_env.md) and
  [`unload()`](https://pkgload.r-lib.org/reference/unload.md) now take a
  package name rather than a path or a `package` object.

- [`run_example()`](https://pkgload.r-lib.org/reference/dev_example.md)
  now works on R 3.1.

- [`unload()`](https://pkgload.r-lib.org/reference/unload.md) now
...
- [`load_all()`](https://pkgload.r-lib.org/reference/load_all.md) gains
  a `helpers` option to specify whether or not to source testthat
  helpers. ([@pitakakariki](https://github.com/pitakakariki) devtools
  [\#1202](https://github.com/r-lib/pkgload/issues/1202))

- [`load_all()`](https://pkgload.r-lib.org/reference/load_all.md) now
  sources the testthat helpers in the namespace environment rather than
  the package environment
  ([\#40](https://github.com/r-lib/pkgload/issues/40)).

- [`load_all()`](https://pkgload.r-lib.org/reference/load_all.md) now
  sets the `NOT_CRAN` environment variable when it sources testthat
...
- [`dev_help()`](https://pkgload.r-lib.org/reference/dev_help.md) now
  optionally takes a character vector of packages to search within. This
  replaces `find_topic()`.

- [`dev_topic_index_reset()`](https://pkgload.r-lib.org/reference/dev_help.md)
